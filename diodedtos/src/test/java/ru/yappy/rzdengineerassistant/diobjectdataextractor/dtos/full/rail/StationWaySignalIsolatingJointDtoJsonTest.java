...
@JsonTest
public class StationWaySignalIsolatingJointDtoJsonTest {
    @Autowired
    private JacksonTester<StationWaySignalIsolatingJointDto> jackson;
    private final StationWaySignalIsolatingJointDto stationWaySignalIsolatingJointDto = (StationWaySignalIsolatingJointDto)
            DiodeTestDataCreator.createJointDto(WayType.STATION_WAY, JointType.ISOLATING_SIGNAL);

    @Test
    public void writeAndRead_whenGetCorrectJson_thenCanParseToDto() throws IOException {
        JsonContent<StationWaySignalIsolatingJointDto> result = jackson.write(stationWaySignalIsolatingJointDto);
        StationWaySignalIsolatingJointDto parsedDto = jackson.parse(result.getJson()).getObject();

        assertThat(parsedDto).isNotNull();
        assertThat(parsedDto).isEqualTo(stationWaySignalIsolatingJointDto);
    }
